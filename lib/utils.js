...
Collection.prototype.children = function children() {
  ...
  if (arguments[0] && typeof arguments[0] === 'object') {
    children =  children.merge(arguments[0]);
  ...
  return this.__queryNode ? this.__queryNode.setIn(selector(this.__query).concat('children'), children) : this.setIn(['children'], children);
}
...
Collection.prototype.props = function props() {
  ...
  return this.__queryNode ? this.__queryNode.setIn(selector(this.__query).concat('props'), props) : (new Map()).set(tagName, this.setIn(['props'], props));
}

Collection.prototype.style = function style() {
  const tagName = this.findKey(() => { return true });
  let style = this.getIn([tagName, 'props', 'style']) || fromJS({props: {style: {}}});
  if (!style) {
    return new Map();
  }
  if (arguments.length === 0) {
  ...
  } else if (typeof arguments[0] === 'string' && typeof arguments[1] === null) {
    style = style.delete(arguments[0]);
  } else if (arguments.length === 1 && typeof arguments[0] === 'string') {

    return style.get(arguments[0]);
  }
  return this.__queryNode ? this.__queryNode.setIn(selector(this.__query).concat(['props', 'style']), style) : (new Map()).set(tagName, this.setIn(['props', 'style'], style));
}

Collection.prototype.events = function events() {
  const tagName = this.findKey(() => { return true });
  let events = this.getIn([tagName, 'props', 'events']) || fromJS({props: {events: {}}});
  if (!events) {
    return new Map();
  }
  if (arguments.length === 0) {
  ...
  return this.__queryNode ? this.__queryNode.setIn(selector(this.__query).concat(['props', 'events']), events) : (new Map()).set(tagName, this.setIn(['props', 'events'], events));
}

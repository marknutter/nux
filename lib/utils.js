...
Collection.prototype.style = function style() {
  ...
  if (typeof arguments[0] === 'object') {
    ...
    style = style.get(arguments[0]);
  }
  return this.__queryNode ? this.__queryNode.mergeIn(selector(this.__query).concat('style'), style) : style;
}

Collection.prototype.styleIn = function styleIn(query) {
  const pathArray = [...selector(query), 'props', 'style'];
  let args = Array.prototype.slice.call(arguments, 1);
...
Collection.prototype.eventsIn = function eventsIn(query) {
  const pathArray = [...selector(query), 'props', 'events'];
  let args = Array.prototype.slice.call(arguments, 1);
...
export function selector(selectorString) {
  ...
  var fullDomPathArray = domPathArray.reduce(function(cur, val, index) {
    if (val === 'ui' || val === 'children' || val === 'props' || domPathArray.length === index+1) {
      return cur.concat([val]);
    } else {
      return cur.concat([val, 'children']);
    }
  },[]);
  var toConcat = [];
